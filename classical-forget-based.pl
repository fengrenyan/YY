...
%---------------cyclic solving------------------------------
snc :-
	gain_filename(filename, L),
	%gain_filename(less_filename, L),
	open('result40to50.txt', write, Str),
	compute(L, R, Str),
	close(Str),
	write("succeed"), nl.
%-----------end cyclic solving-----------------------------
...
newCall(Filename , Variable_n, Clause_num, PN) :-
	%string_concat('all_result_ShareSubF/', Variable_n, F1),
	%string_concat('all_result_ShareSubF', '/Plength_', F2),
	%string_concat(F2, PN, F3),
	%string_concat(F3, '/', F4),
	%string_concat(F4, Filename, Filename1),
	%string_concat('result/', Filename, Filename1),
...
%-------------------------begin----------------------------
%----------compute one in C-----2017-5-16------
/*newCall(Filename , P, Clause_num, PN) :-
    set_prolog_stack(local, limit(137438953472)),  %16G
    set_prolog_stack(global, limit(137438953472)),
	string_concat('result_P1/Variable_', Clause_num, F3),
	string_concat(F3, '/', F4),
	string_concat(F4, Filename, Filename1),
	string_concat(Filename1, '.txt', Filename2),
	open(Filename2, write, Str), 
	read_formula(Formula), 
	gain_prop(Formula, PN1),
	difference_list(P, PN1, P1), %P is the atom of varphi and PN1 is the atom of formula
	write("P1="), write(P1), nl,
        write("T="), write(Formula), nl,
	add_propTerm(PN1), !,
	catch(time_output_1(kForget(Formula, P1, Result), U, V, W),E, false),
	write(Str,'P1='), write(Str, P1), nl(Str), %write(Str,"\n"),
	write(Str,'T='), write(Str, Formula), nl(Str), %write(Str,"\n"),
	write(Str,'UsedInf:'), write(Str, U), nl(Str), %write(Str,"\n"),
	write(Str,'UsedTime:'), write(Str, V), nl(Str), %write(Str,"\n"),
...
read_formula(Formula) :- write_ln('input: '), read(user_input,Input), consult(Input),
	string_concat(Input, '.sht', Filename1),
	string_to_atom(Filename1, Filename),
...
kForget(F,P, NEW) :-
	%trim_stacks,
	degree(F, N), N < 1, !, trace,
	new_wff2cnf(F, CNF),
	forget(CNF, P, NEW1),
	forget_result_simplify(NEW1,NEW2),
	cnf2wff(NEW2, NEW).
kForget(F,P,NEWF) :- first_degree(F, Y), !, trace,  write("Y="), write(Y), nl,
	new_substitute_allk(Y, Y1, 1), trace, write("Y1="), write(Y1), nl,
	new_substitute_allb(Y1, Y2, 1), trace, write("Y2="), write(Y2), nl,
	new_wff2dnf(Y2, DNF),
	simply_dnf(DNF, F1),
	kforget(F1,P,NEW),
