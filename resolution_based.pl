...
%----------------------------kForget----------------------------------------
%kForget(F,[],F) :- !.
%kForget(F, LP, NEW) :- mcnf2list(F, List), kforget(List, LP, NEW1), supp(NEW1, LP, NEW).

kForget(F,[],F) :- !.
kForget(F, LP, NEW) :- mcnf2list(F, List), find_P1(List, LP, List1),
...
devive(C, L1, L2, L3) :- kbc_len(C, Klen, Blen, 0, 0) , trace, generate_KBterm(Klen, Blen) , trace,
	Klen > 0, Blen > 0 ,
...
devive(C, L1, L2, L3) :- klist(KL), write("KL="), write(KL), nl, blist(BL), write("BL="), write(BL), nl, 
	trace, kbc_len(C, Klen, Blen, 0, 0) , trace, Klen == 0, Blen > 0,  trace,
...
devive(C, L1, L2, L3) :- klist(KL), write("KL="), write(KL), nl, blist(BL), write("BL="), write(BL), nl,
	trace, kbc_len(C, Klen, Blen, 0, 0), trace, Blen == 0, Klen > 0 ,
	generate_list(1, Klen, L2) , trace, 
	change2Kterm(L2, KL) ,
	assert(klist(KL)) ,
...
devive(C, L1, L2, L3) :- trace, klist(KL), write("KL="), write(KL), nl, blist(BL), write("BL="), write(BL), nl,
	wff2cnf(C, CNF) ,
	write("CNF="), write(CNF), nl,
	CNF = [Clause],
	split_k(Clause, L1, L2, L3).
...
new_substitute_allk(Term, NewTerm, N1) :- trace, getTermk(Term1,N1,L),
	trace, substitute(@P,Term, Term1, F, Flag), trace,
	assert(pair(@P,Term1)),
	assert(prop(Term1)),
	(Flag==true, N2 is N1+1; N2 is N1),
	(F == Term, NewTerm = F; new_substitute_allk(F, NewTerm, N2)).

new_substitute_allb(Term, NewTerm, N1) :- trace, getTermb(Term1,N1,L), trace,
	substitute($P,Term, Term1, F, Flag), trace,
	assert(pair($P,Term1)),
	assert(prop(Term1)),
	(Flag==true, N2 is N1+1; N2 is N1),
